package com.cocotreedebug.android.ui.widget.slidebar;
...
//    private boolean mSeekBarCanClick;
    ...
    public SlideBarWithTime(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView(context, attrs);
    }

    public SlideBarWithTime(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        this(context, attrs);
    }

    public SlideBarWithTime(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
...
//        int i = mPaddingHorizWithTips + width / 100 * progress   + mSeekBar.getThumb().getIntrinsicWidth() / 2 + mHorizPadding/2 - mTips.getWidth() / 2;
    ...
    private void setupSeekBar() {
        ...
        try {
            ...
            mMaxHeight.set(mSeekBar, UIUtils.dip2px(15));
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        mSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                /*if (selectMode == 0 && !mSeekBarCanClick) {
                    if (progress > oldsign + 5 || progress < oldsign - 5) {
...
//        mTips.setBackgroundResource(mTipsResId);
    ...
    private void updateView() {
        if (mConvertor != null) {
            ...
            if (minimumSetUped) {
                mSolidMini.setText((int) minimum + "bulan");
            }
            if (maxSetuped) {
                mSolidMaxi.setText((int) maximum + "bulan");
            }
            if (minimumSetUped && maxSetuped) {
                mTips.setText((int) getCurrentValue() + "bulan");
            }
        }
    }
    ...
    public double getCurrentValue() {
        if (mConvertor == null) {
            ...
            if (progress >= 0 && progress < 33.3) {
                dayValue = 1;
            } else if (progress >= 33.3 && progress < 33.3 * 2) {
                dayValue = 2;
            } else {
                dayValue = 3;
            }
