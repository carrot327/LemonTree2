...
public class SlideBar extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mText = attributes.getText(R.styleable.SlideBar_slider_text);
        mTextColor = attributes.getColor(R.styleable.SlideBar_slider_textColor, Color.BLACK);
        mtextSize = attributes.getDimensionPixelSize(R.styleable.SlideBar_slider_textSize, 15);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (mBarRect.left == mMeasuredWidth - mBarLength - getPaddingRight() - mBorderStrong) {
                    if (mOnSlideDrag != null) {
                        mOnSlideDrag.onDragEnd(this, true, clickCount);
                    }
                } else {
                    if (mOnSlideDrag != null) {
                        mOnSlideDrag.onDragEnd(this, false, clickCount);
                    }
                    if (mAutoBack) {
                        mBarRect.set(0.0f + getPaddingLeft() + mBorderStrong, 0.0f + getPaddingTop() + mBorderStrong, mBarLength + getPaddingLeft() + mBorderStrong, mMeasuredHeight - getPaddingBottom() - mBorderStrong);
                        invalidate();
